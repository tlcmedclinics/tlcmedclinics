 ...
 *
 * Two things live here. First, how to build a page's `<head>`: canonical URL,
 * OpenGraph card, Twitter card. Second, the structured data (JSON-LD) that
 * lets Google understand *what* a page is — a clinic, a treatment, an article
 * — rather than guessing from the prose.
 *
 * It is one file on purpose. Canonicals and OpenGraph images have to be
 * absolute URLs, and the single most common SEO bug is one page building that
 * URL slightly differently from another (trailing slash, http vs https,
 * preview domain vs production). Every page calling the same helper makes that
 ...
 *
 * Read from the environment so a Vercel preview deployment doesn't publish
 * canonical tags pointing at production — which would tell Google the preview
 * *is* production and can quietly de-index the real pages.
 */
...
/**
 * The social card shown when a link is pasted into WhatsApp, Facebook or
 * LinkedIn. 1200×630 is the size every one of them crops to.
 */
...
type PageMeta = {
  ...
  modifiedTime?: string;
  keywords?: string[];
  /** Keeps a page out of the index — auth screens, thank-you pages, panels. */
  noIndex?: boolean;
};
 ...
 * with query strings and from several internal links, and without an explicit
 * canonical Google treats those as separate, competing pages.
 */
...
/** The root metadata — title template, defaults every page inherits. */
export function rootMetadata(): Metadata {
  const title = `${site.name} — Vein, Skin & Mental Health Care in Lahore`;

  return {
    // Without this, every relative URL in metadata (og:image, canonical)
    // silently stays relative, and crawlers ignore relative social images.
    ...
    robots: {
      ...
      googleBot: {
        ...
        // the truncated default — noticeably better click-through on health
    ...
    // Only emitted when the value is actually set, so the tag never renders empty.
 ...
 * Structured data
 *
 * These return plain objects; <JsonLd> serialises them. Types come from
 * schema.org, and Google's Rich Results test is the thing to validate against.
 * ------------------------------------------------------------------ */
...
/**
 * The clinic itself. `MedicalClinic` is a subtype of LocalBusiness, so this
 * one object feeds both the local pack ("clinic near me") and the knowledge
 * panel. Emitted once, site-wide, from the root layout.
 */
export function clinicSchema() {
  return {
    ...
    // Telemedicine is a genuine differentiator and Google surfaces it as an
    // attribute on the listing, so it's worth stating explicitly.
    availableChannel: {
      "@type": "ServiceChannel",
      name: "Telemedicine",
...
/** The site as a whole — mainly so Google shows the right name in results. */
